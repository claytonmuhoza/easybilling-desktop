...
interface ProductContextProps {
  ...
  openModal: (view: string, prodcuit?: Produit) => void
  closeModal: () => void
}
...
export const ProductProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const [produit, setProduit] = useState<Produit>() //explain this line ?
  const [view, setView] = useState<string>('')

  const closeModal = (): void => {
    setProductModalState(false)
  }
  const openModal = (view: string = 'details', produit?: Produit): void => {
